...
- `cloakbrowser==0.4.10` is installed in the isolated
  `~/.local/share/rldyour/cloakbrowser/.venv` environment.
...
`~/.local/share/rldyour/browser-stack/npm` and managed PATH wrappers below
`~/.local/bin`:
...
- `playwright-cli` `0.1.17` loads the managed `browser.cdpEndpoint` config in
  ...
  config, browser, profile, or extension values, and stock-browser installs.
- Both Node providers resolve from `templates/browser/provider/bun.lock` with
  `bun install --frozen-lockfile --ignore-scripts`.
- `webwright` commit
  `4a46f282ec37f27d6003cc498a977939d62d9015` composes the `base`, live
  `local_browser`, and default OpenAI model configs, then applies the managed
  `local_cdp` overlay last with `local_cdp_auto_start: false`. Its open upstream
  dependency ranges are resolved by the tracked universal
  `templates/browser/webwright-uv.lock` and installed with `uv sync --frozen`.

Every browser action runs `cloakbrowser-cdp-health` first. Diagnostic/version
commands may run without a live browser, but they cannot start one.
...
The production bootstrap pins provider versions, the Webwright repository and
commit, and the managed installation paths in source. It rejects upstream
development overrides that can replace the signed browser path:
...
Existing unmanaged files, symlinks, directories, dirty Webwright checkouts, and
configs are preserved; the browser installation fails instead of replacing
them. Existing global npm/bun installations are not removed.
...
The first command is the runtime authority. A missing service, changed endpoint,
unexpected process command, invalid discovery document, or unreachable port is
a hard failure.
